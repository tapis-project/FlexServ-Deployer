//! Pod-based deployment implementation for FlexServ.
//!
//! This module handles deployment to Kubernetes/Tapis Pods, including
//! volume management, pod configuration, and lifecycle operations.
...
pub struct PodDeploymentOptions {
    ...
    /// so you can create multiple pods for the same model. Id is normalized to lowercase alphanumeric (e.g. UUID without dashes).
...
impl FlexServPodDeployment {
    ...
    pub(crate) fn pod_url_from_result(
        result: &tapis_sdk::pods::models::PodResponseModel,
    ) -> Option<String> {
        result
            .networking
            .as_ref()
    ...
    async fn create_impl(&mut self) -> Result<DeploymentResult, DeploymentError> {
        ...
        let skip_model_download =
            self.server.default_model.is_empty() || self.server.default_model == "no-model-yet";
        let model_id_for_pod = if skip_model_download {
            String::new()
        } else {
            self.server.default_model.clone()
        };
        let model_revision = self.server.model_revision.as_deref().unwrap_or("main");
        let hf_token = self
            .server
            .hf_token
            .clone()
...
mod tests {
    ...
    fn test_map_pods_error_response_codes() {
        ...
        assert!(matches!(
            mapped,
            DeploymentError::TapisInternalServerError(_)
        ));
    }
