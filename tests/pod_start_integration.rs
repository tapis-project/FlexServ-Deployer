//! Integration tests for Pod deployment: start operation (stopped -> running).
//!
//! Pod must be in STOPPED state; run the stop test first if needed.
//!
...
//!
//! Optional: FLEXSERV_NO_MODEL=1 — use no-model config (for demos; matches create with no model).
...
/// Test start(): one call, stopped -> running. Verify return values.
#[test]
fn test_start() {
    ...
    let start_result = deployment.start().expect("start should succeed (pod must be STOPPED)");
    match start_result {
